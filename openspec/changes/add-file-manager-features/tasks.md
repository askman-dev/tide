...
- [ ] 10.1 拖拉右边栏性能问题
  - 无标签时拖动流畅，打开多个标签后拖动不跟手
  - 根因：text_editor 组件在布局变化时重新渲染
  - 建议：后续优化 text_editor 渲染策略或使用虚拟化

- [ ] 10.2 大文件滚动卡顿
  - 当前策略：>1MB 截断到 100KB，仍可能卡顿（~2000-5000 行）
  - 建议：进一步减小截断阈值（如 50KB）或实现虚拟滚动
  - 新需求：考虑单独的"大文件优化"需求

- [ ] 10.3 语法高亮（新需求）
  - 当前 Non-Goals，需要新开需求 `add-syntax-highlighting`
  - 实现需要 tree-sitter 或其他语法解析库